...
import {type AfterMiddleware, type BeforeMiddleware, type ErrorMiddleware, type Middleware} from './types.js';
...
> {
   ...
   */
	private async runThisBefore<T>(event: Input, traceKey?: string): Promise<T> {
		if (this.middleware.before == null) {
   ...
   */
	private async runThisAfter<U>(event: U, traceKey?: string): Promise<Output> {
		if (this.middleware.after == null) {
			return event as unknown as Output;
